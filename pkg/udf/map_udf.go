...
func (u *MapUDFProcessor) Start(ctx context.Context) error {
	...
	var (
		...
		mapBatchHandler    *rpc.GRPCBasedMapBatch
	...
	if enableMapUdfStream {
		...
		serverInfo, err := sdkserverinfo.SDKServerInfo(sdkserverinfo.WithServerInfoFilePath(sdkserverinfo.MapBatchServerInfoFile))
		if err != nil {
			return err
		}
		mapBatchClient, err := mapbatch.New(serverInfo, sdkclient.WithMaxMessageSize(maxMessageSize))
		if err != nil {
			return fmt.Errorf("failed to create map batch client, %w", err)
		}
	...
	for index, bufferPartition := range fromBuffer {
		...
		df, err := forward.NewInterStepDataForward(u.VertexInstance, readers[index], writers, conditionalForwarder, mapHandler, mapBatchHandler, mapStreamHandler, fetchWatermark, publishWatermark, idleManager, opts...)
		if err != nil {
			return err
		}
		finalWg.Add(1)
