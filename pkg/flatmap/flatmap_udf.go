...
func (u *FlatMapUDFProcessor) Start(ctx context.Context) error {
	...
	// TODO(stream) : what all do we need here?
	// mapBatchHandler maybe
	var (
	...
	// TODO(stream): Maybe this annotation should be for Batching instead
	enableFlatmapUdfStream, err := u.VertexInstance.Vertex.FlatmapUdfStreamEnabled()
	if err != nil {
	...
	for index, bufferPartition := range fromBuffer {
		...
		opts := []forward.Option{forward.WithLogger(log),
			forward.WithUDFStreaming(enableFlatmapUdfStream)}
	...
	// TODO(stream): might use for batching
	//else {
